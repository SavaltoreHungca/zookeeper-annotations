 ...
 *
 * 该类是 zk 的客户端程序，要连接 zk 服务需要通过该类。该类在一般情况下是线程安全的
 *
 * <p>
 ...
 * the session valid.
 *
 * 一旦和服务器的连接建立，会为客户端分配 sessionid
 * client 会定期发送心跳检查以保持连接正常
 *
 * <p>
 * The application can call ZooKeeper APIs through a client as long as the
 * session ID of the client remains valid.
 *
 * 只要 sessionid 没有过期，就能通过客户端调用服务器的api
 *
 * <p>
 ...
 *
 * 如果客户端和服务器失联导致 sessionid 失效了，你必须再建个客户端来和服务器通讯
 *
 * <p>
 ...
 *
 * 如果断连，在 sessionid 失效前，zkclient 会尝试连接集群中的其他服务器
 *
 * <p>
 ...
 *
 * 如果失联后重连上了，期间的事件就丢了，然后会发送一个特殊的事件告诉你失联了 即：This special event has
 * EventType None and KeeperState Disconnected
 *
 */
...
public class ZooKeeper {
    ...
    private static class ZKWatchManager implements ClientWatchManager {
        ...
        private final Map<String, Set<Watcher>> dataWatches =
            new HashMap<String, Set<Watcher>>(); // 数据变化的 Watcher
        private final Map<String, Set<Watcher>> existWatches =
            new HashMap<String, Set<Watcher>>(); // 节点是否存在的 Watcher
        private final Map<String, Set<Watcher>> childWatches =
            new HashMap<String, Set<Watcher>>(); // 子节点变化的 Watcher
                                        ...
                                        String clientPath)
        {
            ...
            switch (type) { // 其实就是根据 type 来区分需要被通知的 watcher
            case None:
    ...
    abstract class WatchRegistration {
        ...
        /**
         * 通过 org.apache.zookeeper.ZooKeeper.WatchRegistration#shouldAddWatch(int)
         * 判断是否应该添加 watcher
         *
         * 通过 org.apache.zookeeper.ZooKeeper.WatchRegistration#getWatches(int)
         * 获取 watcher 将要被添加进的集合
         *
         * rc 表示的是 result code
         *
     ...
     *
     * 该构造器是非阻塞创建客户端
     * 在该构造器返回之前，传入的 watcher 随时可能会收到通知
     *
     * <p>
     ...
     *
     * 从传入的服务器列表中随机选择一个连接，会一直尝试直到成功，或者明确指定关闭
     *
     * <p>
     ...
     *
     * linux chroot 特性得到支持，什么是 chroot 请自行 google
     *
     * @param connectString
     ...
     *
     *            服务器地址列表， 这样传 127.0.0.1:3000,127.0.0.1:3001,127.0.0.1:3002
    ...
    /**
     * 关闭会话，sessionid 将会失效，同时 ephemeral 节点将会被删除
     *
     ...
     *
     * 创建一个节点，并赋予给定的 acl 访问权限
     *
     * <p>
     * The flags argument specifies whether the created node will be ephemeral
     * or not.
     *
     * 这段描述里的 flag 指的是 CreateMode 类中的 flag
     * 这里的 flag 其实就是说创建节点的模式
     *
     * <p>
        ...
        throws KeeperException, InterruptedException
    {
        ...
        PathUtils.validatePath(clientPath, createMode.isSequential()); // 校验路径是否合法
        ...
        h.setType(ZooDefs.OpCode.create); // 设置类型
        CreateRequest request = new CreateRequest();  // 创建节点的请求
        CreateResponse response = new CreateResponse(); // 创建请求的响应体
        request.setData(data); // 将节点的数据塞入请求
        request.setFlags(createMode.toFlag());
        request.setPath(serverPath); // 将节点路径塞入请求

        if (acl != null && acl.size() == 0) {
            throw new KeeperException.InvalidACLException();
        }
        request.setAcl(acl); // 设置该节点的访问权限
        ReplyHeader r = cnxn.submitRequest(h, request, response, null); // 直接提交提交请求
        if (r.getErr() != 0) { // 校验返回头信息
            throw KeeperException.create(KeeperException.Code.get(r.getErr()),
                    clientPath);
        }
        if (cnxn.chrootPath == null) {
            return response.getPath(); // 返回创建好的节点的路径
        } else {
            ...
            CreateMode createMode,  StringCallback cb, Object ctx)
    {
        ...
        cnxn.queuePacket(h, r, request, response, cb, clientPath,
                serverPath, ctx, null); // 放入请求队列，和阻塞调用唯一不同的是调用了请求队列
    }

    /**
     * 删除节点，只有在给定的版本和服务器匹配才能删除成功
     *
     ...
     * thrown.
     *
     * 一次请求中所有的 setData 塞入的数据量大小默认不得超过 1mb，这个可以在服务器端设置该限制
     *
    ...
    public List<OpResult> multi(Iterable<Op> ops) throws InterruptedException, KeeperException {
        for (Op op : ops) {
            op.validate(); // 校验是否合法, 实际就是调用了 PathUtils.validatePath(path);
        }
    ...
    public void multi(Iterable<Op> ops, MultiCallback cb, Object ctx) {
        // 同样是调用了 PathUtils.validatePath(path); 验证合法性，如果不合法，将异常信息放入 results
    ...
    private MultiTransactionRecord generateMultiTransaction(Iterable<Op> ops) {
        ...
        for (Op op : ops) {
            transaction.add(withRootPrefix(op)); // 多个请求添加到一个事务中
        }
    ...
    protected List<OpResult> multiInternal(MultiTransactionRecord request)
        throws InterruptedException, KeeperException {
        RequestHeader h = new RequestHeader();
        h.setType(ZooDefs.OpCode.multi); // 请求类型为 multi
        MultiResponse response = new MultiResponse();
        ...
        throws KeeperException, InterruptedException
    {
        ...
        if (watcher != null) {
            // 该类型的注册器通过判断返回 code 来决定 是向 客户端 WatchManager 的 dataWatches 注册 还是向 existWatches 注册
        ...
        RequestHeader h = new RequestHeader();
        h.setType(ZooDefs.OpCode.exists); // 请求类型为校验是否存在
     ...
     * the data on the node.
     *
     * 如果 watch 参数填 true，就会使用默认的 Watcher 来监听该节点的 creates/delete/update
     *
     * @param path
     *                the node path
     * @param watch
    ...
    private static ClientCnxnSocket getClientCnxnSocket() throws IOException {
        // 检查是否有指定和服务器通讯的 Socket 类
        // 没有就使用 ClientCnxnSocketNIO

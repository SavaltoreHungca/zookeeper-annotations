 ...
 *
 * 该请求常作为第一个请求.
 * 通常配合任意类型的事务请求来改变系统状态
 *
 * 该线程继承了 ZooKeeperCriticalThread，所以发生异常后就会退出服务
 *
 */
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements
    ...
    static boolean skipACL; // 是否跳过 acl 权限控制
    static {
    ...
    public void run() {
        try {
            ...
            handleException(this.getName(), e); // 终止程序
        } catch (Exception e) {
    ...
    /**
     * 根据节点路径获取节点的Record，
     * 其首先会从outstandingChangesForPath中获取路径对应的Record，
     * 若未获取成功，则从Zookeeper数据库中获取
     * @param path
     * @return
     ...
     *
     * 该方法也是单线程调用的
     * pRequest2Txn会根据不同的请求类型进行不同的验证，
     * 如对创建节点而言，其会进行会话验证，ACL列表验证，
     * 节点路径验证及判断创建节点的类型（顺序节点、临时节点等）而进行不同操作，
     * 同时还会使父节点的子节点数目加1，
     ...
     * @param request
     * @param record
     * @param deserialize 标志 request 对象是否已经完成反序列化
     */
        ...
        throws KeeperException, IOException, RequestProcessorException
    {
        ...
        switch (type) {
            case OpCode.create:                
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner()); // 检查会话
                CreateRequest createRequest = (CreateRequest)record;
                if(deserialize)
                    ByteBufferInputStream.byteBuffer2Record(request.request, createRequest);
                String path = createRequest.getPath();
                int lastSlash = path.lastIndexOf('/'); // 检查路径是否合法
                if (lastSlash == -1 || path.indexOf('\0') != -1 || failCreate) {
                    LOG.info("Invalid path " + path + " with session 0x" +
                            Long.toHexString(request.sessionId));
                    throw new KeeperException.BadArgumentsException(path);
                }
                List<ACL> listACL = removeDuplicates(createRequest.getAcl()); // 移除重复元素
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        try {
            ...
            case OpCode.multi: // 多事务请求
                ...
                long zxid = zks.getNextZxid(); // 生成事务 id
                ...
                int index = 0;
                for(Op op: multiRequest) {
                    Record subrequest = op.toRequestRecord() ;  // 转换为实体类
                    ...
                    //       我本不想在这里将其序列化然后立即在下一个处理器立即反序列化
                    //       immediately deserialize in next processor. But I'm 
                    //       not sure how else to get the txn stored into our list.
                    //       但是我不知道有没有其他方法将 txn 存储到我们的列表
        ...
        nextProcessor.processRequest(request); // 传递给下一个请求处理器
    }
     ...
     *
     * 该方法确保 acl 没有空，没有非法的权限策略和id，以及扩展相关的 id 到请求的验证信息上
     *
    ...
    /**
     * 将请求添加至submittedRequests队列进行后续处理（run函数中）
     * @param request
     */

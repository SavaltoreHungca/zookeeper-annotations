...
public class SerializeUtils {
    ...
    public static Record deserializeTxn(byte txnBytes[], TxnHeader hdr)
        ...
        bais.mark(bais.available());  // 这里传入任意参数都ok
        Record txn = null;
        switch (hdr.getType()) {  // 确定事务类型
        case OpCode.createSession:
            // This isn't really an error txn; it just has the same
            // format. The error represents the timeout
            txn = new CreateSessionTxn(); // CreateSessionTxn 自动生成的类
            break;
        ...
        if (txn != null) {
            try {
                txn.deserialize(ia, "txn"); // 不同的事务类型采用不同的序列化方式
            } catch(EOFException e) {
                // perhaps this is a V0 Create
                if (hdr.getType() == OpCode.create) {
                    CreateTxn create = (CreateTxn)txn;
                    bais.reset();  // 将 bais 读取位置重新置为 0
